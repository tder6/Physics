...
\vspace{10pt}
\begin{itemize}
\item \blue{运动学}（kinematics），即只研究运动而不涉及运动和力的关系的力学分支。
\item \blue{动力学}（dynamics），即研究运动和力的关系的力学分支。
\end{itemize}

\subsection{牛顿第一定律}
\begin{itemize}
\item \blue{牛顿第一定律}（Newton's first law），即一切物体总保持匀速直线运动状态或静止状态，除非作用在它上面的力迫使它改变这种状态。
...
\item 物体保持原来匀速直线运动状态或静止状态的性质叫做\blue{惯性}（inertia）。牛顿第一定律也叫做\blue{惯性定律}。
...
\subsection{牛顿第二定律}
\begin{itemize}
\item \blue{牛顿第二定律}（Newton's second law），即物体加速度的大小跟它受到的作用力成正比，跟它的质量成反比，加速度的方向跟作用力的方向相同，即：
...
\item \blue{质量}（mass）是描述物体惯性大小（物体运动状态变化难易程度）的物理量，也叫做\blue{惯性质量}。
...
\item 物体对支持物的压力（或对悬挂物的拉力）大于物体所受重力的现象叫做\blue{超重}（overweight）。此时，\blue{视重大于实重}，即 $F_{\text{N}}=m(g+a)>mg$。
\item 物体对支持物的压力（或对悬挂物的拉力）小于物体所受重力的现象叫做\blue{失重}（weightlessness）。此时，\blue{视重小于实重}，即 $F_{\text{N}}=m(g-a)<mg$。
...
\item 基本单位和导出单位一起组成一个\blue{单位制}（systems of units）。
\item 1960 年第 11 届国际计量大会制定了一种国际通用的、包括一切计量领域的单位制，叫做\blue{国际单位制}（Le Système Internation d'Unités），简称 \blue{SI}。
