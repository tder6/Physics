...
\item \blue{电阻}（resistance）是表示导体对电流阻碍作用大小的物理量，用 \blue{$\bm R$} 表示. 单位是\blue{欧姆}，简称\blue{欧}，符号是 \blue{$\bm\Omega$}. 常用单位还有千欧（\blue{${\bf k}\bm\Omega$}）、兆欧（\blue{${\bf M}\bm\Omega$}），换算关系为 \blue{${\bf1k}\bm{\Omega=10^3\Omega}$}，\blue{${\bf1M}\bm{\Omega=10^6\Omega}$}.
\end{itemize}